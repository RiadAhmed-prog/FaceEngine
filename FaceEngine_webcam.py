...
try:
    videos_path = '/home/rmedu/Ahsan_Imran/FaceEngine/faceengine/video/EllenDeGeneres/'
...
video_capture = WebcamVideoStream(src="rtsp://admin:Admin123@10.100.100.229").start()
...
# print("hello")
with open("Face Recognition Paper IEEE ACCESS/Outputs/"+label_name+'recognition_times.csv', mode='w') as recognition_file:
    recognition_writer = csv.writer(recognition_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    recognition_writer.writerow(['Recognized Name', 'Recognition Time', 'False Positive'])
    false_positives = {}
    false_positives[label_name] = 0
    ...
    filename = "output_{}.mp4".format(label_name)
    out = cv2.VideoWriter(filename, fourcc, 10.0, (1680, 1280))

    # out = cv2.VideoWriter("output.mp4", fourcc, 10.0, (1680, 1280))
    ...
    while True:
        ...
        frame = cv2.resize(frame, (1680, 1280))
        try:
        ...
        # frame = cv2.hconcat([frames[0], frames[12]])
        ...
        try:
            ...
            # print("face locations", face_locations_m)
            # print("points", points)
            face_locations = []
            face_encodings = []
            ...
            if len(outputs) > 0:
                ...
                if not detection_time["detected"]:
                ...
                for bb in bbox_xyxy:
                    ...
                    if dict_res is not None and dict_res["done"]:
                        ...
                        av_distance = []
                        try:
                            img2_representation = DeepFace.represent(img_pred, model=model, model_name='ArcFace', enforce_detection=False)
                        except Exception as e:
                            print("DeepFace Exception:", e)
                        ...
                        if np.min(av_distance) < 0.56:
                            
                            maxMatchedItem = known_person_info[av_distance.index(np.min(av_distance))]
                            # print("maxMatchedItem", maxMatchedItem["matching_name"])
                        
                            person_name = maxMatchedItem['name']
                            # print("maxMatchedItem",person_name)
                            # if person_name == label_name:
                            #     true_p +=1
                            # else:
                            #     false_p +=1
                            ...
                            if dict_res == None:
                                ...
                                if not dict_res["done"]:
                                    ...
                                    if dict_res["max_match"] == 11:
                                        
                                        
                                        # print("f p f p",false_positives)
                                        # print(f"person_name {person_name} label_name : {label_name}")
                                        ...
                                        recognition_writer.writerow([label_name, end_time - detection_time["start_time"], false_positives[label_name]])
                                        detection_time["detected"] = True
                                        ...
                                        # if label_name != dict_res['name']:
                                        # print("wrong wrong", dict_res["matching_name"])

                                        for item in dict_res["matching_name"]:
                                            if item["name"] != label_name:
                                                false_p = false_p + item["match"]
                    
                                                false_positives[label_name] = false_positives[label_name] + item["match"]
                                        ...
                                        if dict_check == None:
                                            ...
                                            if person_name != label_name and not false_positives["calculated"]:
                                                print(person_name,"False positive increasing.....")
                                                false_positives[label_name] = false_positives[label_name] + 1
                                            else:
                                        ...
                                        # if person_name != label_name and false_positives["calculated"]:
                                        #     print(person_name,"False positive increasing.....")
                                        #     false_positives[label_name] = false_positives[label_name] + 1
                                        # else:
                                        #     false_positives["calculated"] = False
    ...
    # print("checking False Positive value:", false_p)
    video_capture.stop()
    # cap[1].stop()
    out.release()
